...
double Bicycle::calculateCost(double distance, double weight) const {
    if (weight > BICYCLE_LOAD_CAP) {
        throw InvalidException(
            std::format("Bicycle can't carry such a load! Capacity: {} kg, requested: {} kg",
                        static_cast<int>(BICYCLE_LOAD_CAP), static_cast<int>(weight)));
    }
...
double Bicycle::calculatePassengerCost(double distance, int passengers) const {
    if (passengers > 1) {
        throw InvalidException(
            std::format("Bicycle can only carry 1 passenger! Requested: {}", passengers));
    }
